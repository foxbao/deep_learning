...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat=256, n_cfeat=10, height=28):  # cfeat - context features
        ...
        self.down3 = UnetDown(n_feat, n_feat)
        
        self.down4 = UnetDown(n_feat, n_feat)        # down1 #[10, 256, 8, 8]
        self.down5 = UnetDown(n_feat, n_feat)    # down2 #[10, 256, 4,  4]
        self.down6 = UnetDown(n_feat, 2 * n_feat)
    ...
    def forward(self, x, t, c=None):
        ...
        up2 = self.up1(cemb1*up1 + temb1, down6)  # add and multiply embeddings
        up3 = self.up2(cemb2*up2 + temb2, down5)
        up4 = self.up3(cemb3*up3 + temb3, down4)
        up5 = self.up4(cemb4*up4 + temb4, down3)
        up6 = self.up5(cemb5*up5 + temb5, down2)
        up7 = self.up6(cemb6*up6 + temb6, down1)
        out = self.out(torch.cat((up7, x), 1))
        return out

...
class ContextUnet(nn.Module):
    def __init__(self, in_channels, n_feat=256, n_cfeat=10, height=28):  # cfeat - context features
        ...
        self.timeembed_down_4 = EmbedFC(128, 8*n_feat)
        self.timeembed_down_5 = EmbedFC(128, 16*n_feat)

        self.to_vec = nn.Sequential(nn.AvgPool2d((4)), nn.GELU())

        # Embed the timestep and context labels with a one-layer fully connected neural network
        self.timeembed1 = EmbedFC(128, 16*n_feat)
        self.timeembed2 = EmbedFC(128, 8*n_feat)
        self.timeembed3 = EmbedFC(128, 4*n_feat)
        self.timeembed4 = EmbedFC(128, 2*n_feat)
        self.timeembed5 = EmbedFC(128, 1*n_feat)
...
def sample_ddpm_context(n_sample, layout, save_rate=20):
    ...
    for i in range(timesteps, 0, -1):
        ...
        eps = nn_model(samples, time_emb, c=None, layout=None, layout_concate=layout)

...
class ContextUnet(nn.Module):
    ...
    def forward(self, x, t, c=None, layout=None):
        ...
        up2 = self.up1(iemb1*up1 + temb1, down5)  # add and multiply embeddings
        up3 = self.up2(iemb2*up2 + temb2, down4)
        up4 = self.up3(iemb3*up3 + temb3, down3)
        up5 = self.up4(iemb4*up4 + temb4, down2)
        up6 = self.up5(iemb5*up5 + temb5, down1)
...
# dataset = CustomDataset3("data/parking_generate_data", "data/parking_generate_data/data.txt",
#                          "data/parking_layout_data", "data/parking_layout_data/data.txt", transform, null_context=False)
...
# val_dataset = CustomDataset3("data/val_parking_generate_data", "data/val_parking_generate_data/data.txt",
#                              "data/val_parking_layout_data", "data/val_parking_layout_data/data.txt", transform, null_context=False)
...
nn_model.load_state_dict(torch.load(
    f"{save_dir}/model_670_128_context.pth", map_location=device))
nn_model.train()

model_parameters = get_parameter_number(nn_model)
is_training = True
if is_training:
